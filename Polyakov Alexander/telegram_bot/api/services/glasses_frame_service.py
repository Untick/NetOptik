...
from predictors.material_predictor import MaterialPredictor

class GlassesFrameService:
  def __init__(self, glasses_model, frame_model):
    self.glasses_find_model = glasses_model
    self.frame_material_model = frame_model
  ...
  def call(self, image_url):
    ...
    if result == 1:
      frame_image = self.resizer.resize(image, (200, 120))
      material_predictor = MaterialPredictor(self.frame_material_model)
      predict = material_predictor.predict(frame_image)
